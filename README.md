...
- **[psg1_backup.sh](psg1_backup.sh)** — snapshot the device's hand-built state (proot containers, Termux home + keys, the VM image, package list) to the jumpbox. The non-reproducible state that isn't in git; keep the output local (it holds SSH keys).
